      ...
      <section class="mb-8 bg-white rounded-lg p-6 shadow">
        ...
        <div class="flex flex-wrap gap-2 mb-6">
          <button 
            ...
            :class="[
              'px-4 py-2 rounded-full border-2 font-medium transition-colors',
              selectedCategory === category.id
                ? 'bg-cyan-700 border-cyan-700 text-white'
                : 'bg-white border-cyan-700 text-cyan-700 hover:bg-cyan-700 hover:text-white'
            ]"
          >
            {{ category.name }}
          </button>
          ...
          <div v-for="(events, date) in filteredEventsByCategory" :key="date" class="mb-4 border border-gray-100 rounded-lg overflow-hidden">
            ...
            <div v-if="expandedDate === date" class="px-6 py-4 bg-white">
              <div v-for="event in events" :key="event.id" class="mb-6 last:mb-0 bg-gray-50 p-4 rounded-lg shadow-sm">
                ...
                <button class="bg-cyan-700 text-white px-4 py-2 rounded hover:bg-cyan-800 transition-colors">Anmelden</button>
              </div>
            </div>
          </div>
        </div>
