<template>
    <section class="w-full relative">
        <div class="relative w-full">
            <img src="@/assets/images/1.jpg" alt="Hero Background"
                class="w-full h-[60vh] md:h-[800px] object-cover block" />

            <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2
         w-[95vw] max-w-4xl bg-[rgba(8,121,144,0.8)] p-[80px] rounded-[20px] text-center text-white">
                <div class="flex flex-col sm:flex-row items-center justify-center gap-4 mb-4">
                    <img src="@/assets/images/logo1.png" alt="Uri Flag"
                        class="h-16 sm:h-20 rounded shadow-md object-contain" />
                    <h1
                        class="font-inter font-light text-2xl sm:text-4xl md:text-6xl leading-tight flex flex-row gap-3">
                        <span>Griäzi!</span>
                ...
                <div class="greetings grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6 p-2 w-full">
                    <p v-for="(greeting, index) in greetings" :key="index" :class="[
                        'greeting-item text-[2rem] md:text-[1.75rem] font-light flex flex-col items-center gap-1',
                        `pulse-${index % 5}`
                    ]">
                        {{ greeting.text }}
                        <span class="text-base md:text-xl opacity-70 mt-1 font-normal tracking-tight">{{ greeting.lang
                        }}</span>
                    </p>
                </div>
            </div>
